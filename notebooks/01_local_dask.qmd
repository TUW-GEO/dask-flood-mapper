...
## Height Above Nearest Drainage Point from Alaska Satellite Facility

For flood mapping we are only interested in microwave backscattering over what used to be land, as such, we need a way to mask water bodies. For this we use the Height Above Nearest Drainage Point (HAND) from the following Stac catalog.
